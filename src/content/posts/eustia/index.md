...
>
> > 之后居然还是在游戏内发现了硬换行，但属于确实必要，我找出了对应的台本内容：
> > 
> > ```yaml
> > - name: print
> >   line_number: 0
...
默认点击（下一句）会截断当前播放的语音，要改变，将设置的第三栏“文字設定”的第七个设置项“タップボイスカット”（Tap Voice Cut）设置为“オフ”（Off）即可。

### 隐藏对话框

桌面模式没有隐藏对话框的按钮，我只发现了一种在移动端设备隐藏对话框的方法：在平板模式时，设置的第五栏“フリック”（滑动）可以设置屏幕中虚拟摇杆的行为，将向下滑动设置为 WINDOW OFF，即可使用该虚拟摇杆隐藏对话框。我个人平常使用桌面模式，要隐藏对话框截图时切换到平板模式，截图完毕后再切换会桌面模式。
