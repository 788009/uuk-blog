...
import InteractiveFlowchart from '/src/components/InteractiveFlowchart/index.jsx';

export const myMermaidCode = `---
config:
  layout: elk
---
flowchart LR
...
30. Marginal propensity to consume 是 marginal 的，只与增加量有关，与绝对值无关
31. Trade balance 是 current account 的子集，定义为 exports - imports，其正负与 current account 的正负无必然关联
32. The United States national debt is the amount of money owed to holders of United States government securities
33. Disposable income = Income - Tax
34. 预期的变化（如 inflation）只影响 nominal，不影响 real；非预期的变化影响 real
35. 关系图
    1. 实线代表正相关，虚线代表负相关
    2. 只能体现有影响，无法体现影响强度，也无法体现产生影响的条件
    3. 若亮色/暗色明显不匹配，请尝试多次刷新页面或关闭浏览器的自动暗色功能
    4. 交互
        - 选中一个节点：高亮与其相连的边
        - 选中两个节点：高亮从起点到终点的所有路径
        - 选中更多节点：高亮从起点到终点且经过所有指定途径点的所有路径

<InteractiveFlowchart code={myMermaidCode} client:visible />

[Mermaid Code](/records/notes/macro/mermaid-code/)
